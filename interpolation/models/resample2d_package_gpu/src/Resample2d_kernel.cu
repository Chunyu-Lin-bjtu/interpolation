...
__global__ void kernelForward_Resample2d_updateOutput(const int n, const float* input1, const long4 input1_size, const long4 input1_stride,
    const float* input2, const long4 input2_size, const long4 input2_stride, float* output, const long4 output_size, const long4 output_stride, int kernel_size) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    int dim_b = DIM0(input1_size);
    int dim_c = DIM1(input1_size);
    int dim_h = DIM2(input1_size);
    int dim_w = DIM3(input1_size);
    int dim_chw = dim_c * dim_h * dim_w;
    int dim_hw  = dim_h * dim_w;
    ...
    int xf,yf;
    if (dx >0.5)
	xf = max(min( x-1,    dim_w-1), 0);
    if (dx <-0.5)
	xf = max(min( x+1,    dim_w-1), 0);
    if (dy >0.5)
	yf = max(min( y-1,    dim_h-1), 0);
    if (dy <-0.5)
	yf = max(min( y+1,    dim_h-1), 0);

    float val = DIM3_INDEX(input1, b, c, y, x);
    DIM3_INDEX(output, b, c, yf, xf)=val;

}


__global__ void kernelForward_Resample2d_backward_input1(
    const int n, const float* input1, const long4 input1_size, const long4 input1_stride, const float* input2, const long4 input2_size, const long4 input2_stride,
    const float* gradOutput, const long4 gradOutput_size, const long4 gradOutput_stride, float* gradInput, const long4 gradInput_size, const long4 gradInput_stride, int kernel_size) {
    ...
    float val = 1*DIM3_INDEX(gradOutput, b, c, yf, xf);
    DIM3_INDEX(gradInput, b, c, y, x) = val;

}

__global__ void kernelForward_Resample2d_backward_input2(
    const int n, const float* input1, const long4 input1_size, const long4 input1_stride, const float* input2, const long4 input2_size, const long4 input2_stride,
    const float* gradOutput, const long4 gradOutput_size, const long4 gradOutput_stride, float* gradInput, const long4 gradInput_size, const long4 gradInput_stride, int kernel_size) {
...
void Resample2d_kernelForward_forward(THCState* state, THCudaTensor* input1, THCudaTensor* input2, THCudaTensor* output, int kernel_size) {
    int n = 0;
    ...
    n = THCudaTensor_nElement(state, output);
    kernelForward_Resample2d_updateOutput<<< (n + CUDA_NUM_THREADS - 1)/CUDA_NUM_THREADS, CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state) >>>(
        n, THCudaTensor_data(state, input1), input1_size, input1_stride, THCudaTensor_data(state, input2), input2_size, input2_stride,
        THCudaTensor_data(state, output), output_size, output_stride, kernel_size);
...
void Resample2d_kernelForward_backward(THCState* state, THCudaTensor* input1, THCudaTensor* input2, THCudaTensor* gradOutput, THCudaTensor* gradInput1, THCudaTensor* gradInput2, int kernel_size) {
    int n = 0;
    ...
    n = THCudaTensor_nElement(state, gradOutput);
    kernelForward_Resample2d_backward_input1<<< (n + CUDA_NUM_THREADS - 1)/CUDA_NUM_THREADS, CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state) >>>(
        n, THCudaTensor_data(state, input1), input1_size, input1_stride, THCudaTensor_data(state, input2), input2_size, input2_stride,
        THCudaTensor_data(state, gradOutput), gradOutput_size, gradOutput_stride, THCudaTensor_data(state, gradInput1), gradInput1_size, gradInput1_stride, kernel_size
    );
    ...
    n = THCudaTensor_nElement(state, gradInput2);
    kernelForward_Resample2d_backward_input2<<< (n + CUDA_NUM_THREADS - 1)/CUDA_NUM_THREADS, CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state) >>>(
        n, THCudaTensor_data(state, input1), input1_size, input1_stride, THCudaTensor_data(state, input2), input2_size, input2_stride,
        THCudaTensor_data(state, gradOutput), gradOutput_size, gradOutput_stride, THCudaTensor_data(state, gradInput2), gradInput2_size, gradInput2_stride, kernel_size
    );
    THCudaCheck(cudaGetLastError());
